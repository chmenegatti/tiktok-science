...
| Script | `pipeline/script.ts` | `gerarRoteiro()` | Gemini `gemini-2.5-flash` via REST `fetch` (no SDK), **structured output** via `generationConfig.responseSchema`. Returns a `Roteiro` of exactly 10 `Slide`s. Prompt is reach-optimized (scroll-stopper cover, save-worthy body, SEO+CTA caption, exactly 5 hashtags). **Subject dedup**: receives the list of already-published subjects (`src/historico.ts` -> `data/assuntos.json`, committed by publish) as a forbidden list, and returns a short `assunto` string identifying the chosen curiosity; `index.ts` registers it before publishing. Areas may repeat in the rotation, subjects never. |
